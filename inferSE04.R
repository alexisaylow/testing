...
#if you would like it to have only one argument, set Par0 = priors and remove Par0 as an argument

load("loadfornlm.RData") #this contains bestsofar, tryPmatrix, datArW03, and priors
...
beliefModelFit <- function(pts,Par0) {
  ...
  for (ptN in pts){ #1:dim(datArW03)[3] ){
    ...
    mypath <- file.path(baseDir,paste("myplot2_", ptN, ".png", sep = "")) 
    ...
    mypath <- file.path(baseDir,paste("myplot3_", ptN, ".png", sep = "")) 
